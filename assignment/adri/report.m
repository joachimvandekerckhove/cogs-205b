...
function report(data)
    ...
    fid = fopen('adriReport.md','w');
        ...
        fprintf(fid, '\n');   
        fprintf(fid, '- Mean(X<sub>2</sub>) = %.3f \n',thisReport.Mean(2,1));
        fprintf(fid, '\n');
        fprintf(fid, '- Variance(X<sub>1</sub>) = %.3f', thisReport.Covariance(1,1));
        fprintf(fid, '\n');   
        fprintf(fid, '- Variance(X<sub>2</sub>) = %.3f \n', thisReport.Covariance(2,2));
        fprintf(fid, '\n');
        fprintf(fid, '- Covariance(X<sub>1</sub>,X<sub>2</sub>) = %.3f', thisReport.Covariance(1,2));
        fprintf(fid, '\n');
