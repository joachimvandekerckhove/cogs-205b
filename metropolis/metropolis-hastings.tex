...
\begin{itemize}
 ...
 \item MH algorithms need some customization and fine-tuning to be most efficient
...
\begin{exercise}[Metropolis-Hastings sampling in MATLAB]
Write a new class called \texttt{ MetropolisHastings() }

Make the class inherit from \texttt{ handle } (like my HillClimber class, which you can use as a starting point)
...
In addition to the standard methods, \texttt{ MetropolisHastings() } should have at least a method \texttt{ Draw(N) } to draw $N$ samples

You should assume that \texttt{ Target } has exactly one input, but the input may be a vector
